...
@test "build_agent_content : sans langue — pas d'instruction de langue dans l'output" {
  run build_agent_content "$TEST_DIR/agents/test-agent.md"
  [ "$status" -eq 0 ]
  ! echo "$output" | grep -q "Langue de travail"
}

@test "build_agent_content : avec lang=english — instruction de langue présente" {
  run build_agent_content "$TEST_DIR/agents/test-agent.md" "english"
  [ "$status" -eq 0 ]
...
@test "build_agent_content : agent inexistant — retourne exit non-zero" {
  run build_agent_content "$TEST_DIR/agents/inexistant.md"
  [ "$status" -ne 0 ]
}
...
EOF
  ...
  run build_agent_content "$TEST_DIR/agents/developer-frontend.md" "" "$proj"
  [ "$status" -eq 0 ]
  # Les stack skills (Bucket B/native) sont listées dans le guide "Skills disponibles"
  # — leur contenu n'est pas inline. On vérifie la présence dans le tableau natif.
  echo "$output" | grep -q "dev-standards-typescript"
}
...
EOF
  ...
  run build_agent_content "$TEST_DIR/agents/agent-with-ts.md" "" "$proj"
  [ "$status" -eq 0 ]
