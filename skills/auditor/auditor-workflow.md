...
2. **PUIS appeler l'outil `question` pour la validation**
   - Le champ `question` doit commencer par `[Auditeur — Phase X | Projet : <nom>]` si invoqué depuis orchestrateur
...
<contenu complet du récap — contexte chargé, domaines identifiés, sous-agents sélectionnés>
...
### Format de retour final (Phase 4)

**Si CONTEXTE = orchestrateur_feature :**

Produire dans cet ordre :

1. **La synthèse exécutive multi-domaines ou le rapport consolidé** (texte narratif)
