...
description: Compétence d'enrichissement des documents vivants (ONBOARDING.md, CONVENTIONS.md) — à activer après chaque rapport d'audit, diagnostic ou planification. L'agent identifie les découvertes à capitaliser, demande confirmation à l'utilisateur, puis délègue l'écriture au documentarian via l'outil task. Aucune écriture directe.
---
...
Ce skill définit le protocole par lequel les agents d'analyse (auditor coordinateur,
planner, debugger) enrichissent de manière **incrémentale** les fichiers `ONBOARDING.md`
...
1. Consolider les découvertes remontées par les sous-agents
...
à la racine du projet avec les découvertes suivantes issues d'un <audit <domaine> / diagnostic de bug / planification de feature>.
...
- Ajouter une ligne de traçabilité en bas du fichier :
  > Enrichi le <DATE> suite à <audit <domaine> / diagnostic <titre> / planification <feature>> — <agent-id>
...
✅ **Contextualisé** : indiquer la source — `(audit sécurité)`, `(diagnostic auth)`, etc.
